...
export default function WalletDetailPage() {
    ...
    const fetchTransactions = async () => {
      try {
        ...
        setPaginationMeta({
          ...
          hasPrev: paginatedInfo.has_prev || false, // NOTE: The API must return `true` for this on page 2+ for the "Previous" button to work.
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= paginationMeta.totalPages) {
      ...
      // The `window.scrollTo` line has been removed to prevent scrolling to top.
    }
  };
