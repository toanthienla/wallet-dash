    ...
    const fetchTransactions = async () => {
      try {
        ...
        const res = await axiosClient.get(
          `${API_URL}/transaction/dashboard/queues?${params.toString()}`
        )
        ...
        const formatted = data.map((tx: any) => ({
          ...
          type: mapTransactionType(tx.transaction_type?.name || tx.type) as Tx["type"],
          amount: `${tx.amount} ${tx.currency_slug?.toUpperCase() || ""}`.trim(),
          asset: getAssetType(tx.currency),
          status: mapTransactionStatus(tx.transaction_status) as Tx["status"],
        }))
...
function mapTransactionType(type: string): Tx["type"] {
  const typeMap: Record<string, Tx["type"]> = {
    "deposit": "Deposit",
    "withdraw": "Withdraw",
    "redeem": "Redeem",
  }
...
function mapTransactionStatus(status: string): Tx["status"] {
  const statusMap: Record<string, Tx["status"]> = {
...
function getAssetType(currency: any): Tx["asset"] {
  if (!currency) return "Crypto"

  if (currency.is_fiat_token) return "Fiat"
  if (currency.is_point_token) return "Point"
  if (currency.is_crypto_token) return "Crypto"
